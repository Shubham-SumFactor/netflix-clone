...
export const getCommonVideos = async (url) => {
 ...
 //   const data = isDev ? videoTestData : await fetchVideos(url);
    ...
    const isDev = process.env.DEVELOPMENT;
        const is = (isDev === "true") ? true : false;
        const data =  is ? videoTestData : await fetchVideos(url);
...
export const getVideos = (searchQuery) =>{
    const URL = `search?part=snippet&q=${searchQuery}&topicId=%20%09&type=video&videoType=any`
